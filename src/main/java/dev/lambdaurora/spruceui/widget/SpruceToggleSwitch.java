...
public class SpruceToggleSwitch extends AbstractSpruceBooleanButtonWidget {
	...
	protected void renderButton(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		context.drawTexture(RenderLayer::getGuiTextured, TEXTURE, this.getX() + (this.getValue() ? 14 : 0), this.getY() + (this.getHeight() / 2 - 9),
		...
		if (this.showMessage) {
			...
			context.drawTextWithShadow(this.client.textRenderer, message, this.getX() + 36, this.getY() + (this.getHeight() - 8) / 2,
	...
	protected void renderBackground(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		RenderSystem.defaultBlendFunc();
		RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE_MINUS_SRC_ALPHA);
		context.drawTexture(RenderLayer::getGuiTexturedOverlay, TEXTURE, this.getX(), this.getY() + (this.getHeight() / 2 - 9),
