	...
	protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
		if (this.title != null) {
			...
			if (this.width > titleWidth) {
				context.fill(this.getX(), this.getY() + 4, titleX - 5, this.getY() + 6, ColorUtil.TEXT_COLOR);
				context.fill(titleX + titleWidth + 5, this.getY() + 4, this.getX() + this.getWidth(), this.getY() + 6, ColorUtil.TEXT_COLOR);
			}
			context.drawTextWithShadow(this.client.textRenderer, this.title, titleX, this.getY(), ColorUtil.WHITE);
		} else {
			context.fill(this.getX(), this.getY() + 4, this.getX() + this.getWidth(), this.getY() + 6, ColorUtil.TEXT_COLOR);
		}
