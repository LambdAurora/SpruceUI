...
public class SpruceTabbedWidget extends AbstractSpruceParentWidget<SpruceWidget> {
	...
	protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
		if (this.title != null) {
			context.drawCenteredTextWithShadow(this.client.textRenderer, this.title, this.getX() + this.list.getWidth() / 2,
					this.getY() + 6, 0xffffffff);
		}
		this.list.render(context, mouseX, mouseY, delta);
		if (this.list.getCurrentTab() != null)
			this.list.getCurrentTab().container.render(context, mouseX, mouseY, delta);
	}
	...
	public static class TabEntry extends Entry {
		...
		protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
			...
			for (var it = this.title.iterator(); it.hasNext(); y += 9) {
				var line = it.next();
				context.drawText(this.client.textRenderer, line, this.getX() + 4, y, 0xffffff, false);
			}
			if (this.description != null) {
				y += 4;
				for (var it = this.description.iterator(); it.hasNext(); y += 9) {
					var line = it.next();
					context.drawText(this.client.textRenderer, line, this.getX() + 8, y, 0xffffff, false);
				}
			}
		}
